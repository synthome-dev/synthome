import type { UnifiedVideoOptions } from "../schemas/unified.js";
import type {
  BaseGenerateOptions,
  ProviderConfig,
  VideoJob,
...
async function createVideoJob<TModelOptions extends ProviderConfig>(
  ...
  if (!response.ok) {
    let errorMessage = `Failed to create video job: ${response.statusText}`;
    try {
      const errorBody = await response.json();
      if (errorBody.error || errorBody.message) {
        errorMessage = `Failed to create video job: ${errorBody.error || errorBody.message}`;
        if (errorBody.code) {
...
async function fetchJobStatus(jobId: string): Promise<VideoJob> {
  ...
  if (!response.ok) {
    let errorMessage = `Failed to fetch job status: ${response.statusText}`;
    try {
      const errorBody = await response.json();
      if (errorBody.error || errorBody.message) {
        errorMessage = `Failed to fetch job status: ${errorBody.error || errorBody.message}`;
        if (errorBody.code) {
          errorMessage += ` (${errorBody.code})`;
        }
      }
