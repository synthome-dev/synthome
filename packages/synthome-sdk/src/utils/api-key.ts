...
/**
 * Retrieves the SYNTHOME_API_URL from environment variables or options
 * @param providedUrl - Optional API URL provided directly
 * @returns The API URL (defaults to https://api.synthome.dev/api/execute)
 */
export function getSynthomeApiUrl(providedUrl?: string): string {
  // First check if provided directly
  if (providedUrl) {
    return providedUrl;
  }
  ...
  // Then check environment variable
  if (typeof process !== "undefined" && process.env) {
    const envUrl = process.env.SYNTHOME_API_URL;
    if (envUrl) {
      return envUrl;
    }
  }
  ...
  return "https://api.synthome.dev/api/execute";
}
