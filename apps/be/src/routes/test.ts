...
testRouter.get("/", async (c) => {
  try {
    ...
    const e = await executeFromPlan(execution.toJSON(), {
      webhook: "http://localhost:3100/api/test/webhook",
    });
...
testRouter.get("/webhook-async", async (c) => {
  try {
    ...
    return c.json({
      ...
      executionId: execution.id,
      status: execution.status,
      message: "Execution started. Webhook will be called when complete.",
      webhookUrl,
    });
  } catch (error) {
